...
@router.post("/register/{sns_type}", status_code=201, response_class=Token)
async def register(sns_type: SnsType, reg_info: UserRegister, Session = Depends(db.session)):
  '''
  '회원가입 API'\n
  :param sns_type:
  :param reg_info:
  ...
  if sns_type == SnsType.email:
    ...
    if not reg_info.emial or not reg_info.pw:
...
def create_access_token(*, data: dict = None, expires_delta: int = None):
  to_encode = data.copy()
  if expires_delta:
    to_encode.update({"exp": datetime.utcnow() + timedelta(hour=expires_delta)})
  encoded_jwt = jwt.encode(to_encodem, JWT_SECRET, algorithm=JWT_ALGORITHM)
  return encoded_jwt
